...
// P151 AUTO-UPDATE COORDINATOR FINAL
...
function injectUpdateCoordinator(response) {
    ...
    return response.text().then(function(html){
        ...
        var coordinatorUrl=new URL('register-sw.js',new URL(self.registration.scope)).pathname;
        var script='<script id="RAWAEA_UPDATE_COORDINATOR" src="'+coordinatorUrl+'"></script>';
        var marker=html.indexOf('</head>');
        html=marker>=0 ? html.slice(0,marker)+script+html.slice(marker) : script+html;
        return new Response(html,{status:response.status,statusText:response.statusText,headers:new Headers(response.headers)});
    }).catch(function(error){ console.warn('[SW] coordinator injection skipped',error); return response; });
}
